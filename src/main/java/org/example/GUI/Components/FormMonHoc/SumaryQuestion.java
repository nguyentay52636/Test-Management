...
public class SumaryQuestion extends JPanel {
    ...
    private void initComponents() {
        ...
        topicPanel.setBackground(new Color(245, 248, 252));
        GridBagConstraints topicGbc = new GridBagConstraints();
        topicGbc.insets = new Insets(5, 10, 5, 10);

        JLabel lblSubject = new JLabel("Chọn Môn Học:");
        lblSubject.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        lblSubject.setForeground(new Color(66, 80, 102));
        topicGbc.gridx = 0;
        topicGbc.gridy = 0;
        topicPanel.add(lblSubject, topicGbc);

        cboTopic = new JComboBox<>();
        cboTopic.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        cboTopic.setBackground(Color.WHITE);
        cboTopic.setPreferredSize(new Dimension(200, 35));
        cboTopic.addActionListener(e -> loadDataByTopic());
        topicGbc.gridy = 1;
        topicPanel.add(cboTopic, topicGbc);
    ...
    private void loadTopics() {
        ...
        List<TopicsDTO> topics = topicDAO.getAllTopics();
    ...
    private void loadDataByTopic() {
        ...
        if (selectedIndex >= 0 && selectedIndex < topicIDs.size()) {
            int topicID = topicIDs.get(selectedIndex);
            tableModel.setRowCount(0);
            List<QuestionDTO> questions = questionDAO.getQuestionsByTopic(topicID);
            for (QuestionDTO q : questions) {
                tableModel.addRow(new Object[] {
                        q.getQuestionID(),
                        q.getQContent(),
    ...
    public void exportExamToDocx() {
        int selectedIndex = cboTopic.getSelectedIndex();
        if (selectedIndex < 0 || selectedIndex >= topicIDs.size()) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn một chủ đề trước khi xuất đề thi!", "Thông báo",
        ...
        String filePath = "HistoryExam.docx";
        try (XWPFDocument document = new XWPFDocument()) {
            XWPFParagraph title = document.createParagraph();
            title.setAlignment(ParagraphAlignment.CENTER);
            XWPFRun titleRun = title.createRun();
            titleRun.setText("ĐỀ THI TRẮC NGHIỆM MÔN LỊCH SỬ");
